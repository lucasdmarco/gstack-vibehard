...
/**
 * O dream audit destes testes é FIXTURE, não o do repo.
 *
 * Antes eles caíam no audit real e passavam só porque o repo, naquele commit,
 * tinha todo contrato provado — um verde emprestado. Quando o S52.B endureceu a
 * régua e uma claim virou NOT_PROVED, cinco testes que nada têm a ver com o
 * placar começaram a falhar. O `dream-required` reprovando ali estava CERTO; o
 * errado era estes testes dependerem dele. Quem prova o `dream-required` é
 * `publish_guard_prd45.test.js`, que injeta os casos tóxicos de propósito.
 */
const dreamProvado = () => ({
  summary: { REAL: 1, PARTIAL: 0, RISK: 0, PLACEBO: 0, ROADMAP: 0 },
  claims: [{ id: "verify", status: "REAL" }],
})
...
test("publish-guard: tudo ok → pass (tag ausente vira warning, não bloqueia)", async () => {
  ...
  try {
    ...
    const r = publishGuard({ cwd, dream: dreamProvado, exec: gitExec({ tags: ["v2.28.1"] }) })
...
test("publish-guard: working tree suja → fail", async () => {
  ...
  try {
    const { publishGuard } = await imp()
    const r = publishGuard({ cwd, dream: dreamProvado, exec: gitExec({ tags: ["v2.28.1"], porcelain: " M src/a.js\n?? b.js" }) })
...
test("publish-guard: tree suja com >5 arquivos → lista 5 e resume o resto", async () => {
  ...
  try {
    ...
    const r = publishGuard({ cwd, dream: dreamProvado, exec: gitExec({ tags: ["v2.28.1"], porcelain: many }) })
    const detail = r.checks.find((c) => c.id === "tree-clean").detail
...
test("publish-guard: versão não bumpada vs última tag → fail", async () => {
  ...
  try {
    const { publishGuard } = await imp()
    const r = publishGuard({ cwd, dream: dreamProvado, exec: gitExec({ tags: ["v2.28.1", "v2.28.0"] }) })
    assert.equal(r.status, "fail")
...
test("publish-guard: primeira release (sem tags) → version-bump passa", async () => {
  ...
  try {
    const { publishGuard } = await imp()
    const r = publishGuard({ cwd, dream: dreamProvado, exec: gitExec({ tags: [] }) })
    assert.equal(r.status, "pass")
  } finally { await rm(cwd, { recursive: true, force: true }) }
})
...
test("publish-guard: sem package.json version → fail imediato", async () => {
  ...
  try {
    const { publishGuard } = await imp()
    const r = publishGuard({ cwd, dream: dreamProvado, exec: gitExec({}) })
    assert.equal(r.status, "fail")
...
test("publish-guard: gh presente e CI success → ci-green passed", async () => {
  ...
  try {
    const { publishGuard } = await imp()
    const r = publishGuard({ cwd, dream: dreamProvado, exec: gitExec({ tags: ["v2.28.1"], hasGh: true }) })
...
test("publish-guard: qg.py sincronizado com a versão → qg-version passed", async () => {
  ...
  try {
    const { publishGuard } = await imp()
    const r = publishGuard({ cwd, dream: dreamProvado, exec: gitExec({ tags: ["v3.0.16"] }), readQgVersion: () => "3.0.17" })
    assert.equal(r.status, "pass")
...
test("publish-guard: qg.py divergente do package → fail HARD (bloqueia release)", async () => {
  ...
  try {
    const { publishGuard } = await imp()
    const r = publishGuard({ cwd, dream: dreamProvado, exec: gitExec({ tags: ["v3.0.16"] }), readQgVersion: () => "3.0.3" })
    assert.equal(r.status, "fail")
...
test("publish-guard: sem qg.py → qg-version not_applicable (não bloqueia)", async () => {
  ...
  try {
    const { publishGuard } = await imp()
    const r = publishGuard({ cwd, dream: dreamProvado, exec: gitExec({ tags: ["v3.0.16"] }), readQgVersion: () => null })
    assert.equal(r.status, "pass")
...
test("publish-guard: DEFEITO v4.0.0 — commit fora do remoto → fail HARD (bloqueia publish)", async () => {
  ...
  try {
    const { publishGuard } = await imp()
    const r = publishGuard({ cwd, dream: dreamProvado, exec: gitExec({ tags: ["v4.0.0"], parity: { contains: "" } }), readQgVersion: () => "4.0.1" })
    assert.equal(r.status, "fail")
...
test("publish-guard: paridade completa (commit no remoto + tag corresponde) → pass", async () => {
  ...
  try {
    ...
    const r = publishGuard({ cwd, dream: dreamProvado, exec: gitExec({ tags: ["v4.0.0"], parity }), readQgVersion: () => "4.0.1" })
