 ...
 *
 * `runtime-supervisor.js:346` e `:389` são a mesma forma — o seam de escrita:
 *
...
test("REPO: `logsCommand` resolve para leitura de arquivo; `followLog` NÃO", async () => {
  ...
  assert.equal(pts.find((p) => p.line === 389).parameterOrigin, "file_read",
  ...
  assert.equal(pts.find((p) => p.line === 346).parameterOrigin, "unresolved",
