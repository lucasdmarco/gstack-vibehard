...
/**
 * `gstack_vibehard orchestrate <planId> [--verify-with <harness>] [--yes] [--json]`
 * — Meta-Harness MVP (PR13.6): camada sobre worktree+executor com VERIFIER independente
 * e DUPLA VERIFICAÇÃO (reviewer LLM advisory + diff-hygiene determinístico bloqueante).
 * SEM auto-merge: passo `passed` vira branch; o resto é descartado. Tudo no provenance.
 * Reviewer LLM é um HOOK advisory (opts.verifierReview) — sem ele, o gate decide.
 */
export function orchestrateCommand(args = [], opts = {}) {
  const cwd = opts.cwd || process.cwd()
  const json = args.includes("--json")
  const yes = args.includes("--yes")
  const vi = args.indexOf("--verify-with")
  const verifyWith = vi >= 0 ? args[vi + 1] : undefined
  const planId = args.filter((a) => !a.startsWith("-"))[0]
  ...
  const result = runOrchestration({
    ...
    verifierReview: opts.verifierReview || (() => ({ ok: true, advisory: true })), // hook advisory (sem reviewer real = no-op; o gate decide)
    gate: opts.gate || ((step) => {
  ...
  info("  Dupla verificação: reviewer LLM é advisory; o gate determinístico decidiu o status.")
}
