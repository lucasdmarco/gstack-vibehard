...
const STRING_FLAGS = { "--intent": "intent", "--target": "target", "--scope": "scope", "--harness": "harness", "--run": "runId" }
...
function consumeEvidence(args, i, out) {
...
function parseFlags(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    const a = args[i]
    const key = STRING_FLAGS[a]
    if (key) out[key] = args[++i]
    else if (a === "--sensitive") out.sensitive = true
    else if (a === "--evidence") i = consumeEvidence(args, i, out)
  }
  return out
}
 ...
 */
const resolveEnforcement = (opts, f) => opts.enforcement || (f.harness ? getAdapterInfo(f.harness).enforcement : "real_hooks")
function buildAction(opts, f) {
  return opts.action || { intent: f.intent || "unknown", target: { scope: f.scope, pathOrName: f.target, sensitive: !!f.sensitive } }
}

function challengeClassify(action, json) {
...
const grantSuffix = (d) => (d.grantedBy ? `, grant ${d.grantedBy.slice(0, 12)}` : "")
function challengePretool(cwd, action, f, opts, json) {
  ...
  if (decision.decision === "allow") success(`  ✓ allow (risco ${decision.risk}${grantSuffix(decision)})`)
...
function recordChallenge(cwd, action, f, enforcement, decision) {
  try {
    recordAction(cwd, {
...
function renderEvaluate(action, decision) {
...
export function challengeCommand(args = [], opts = {}) {
  ...
  const action = buildAction(opts, f)
  const enforcement = resolveEnforcement(opts, f)
  if (sub === "classify") return challengeClassify(action, json)
  if (sub === "pretool") return challengePretool(cwd, action, f, opts, json)
  return challengeEvaluate(cwd, action, f, opts, enforcement, json)
}
