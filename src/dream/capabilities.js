 ...
 * pelo `verify` (reduced_trust) e pelo `doctor`. É honesta: nenhuma CLI consegue
 * interceptar o stream de render do harness, então `supportsPreOutputInterception`
 * é `false` em todos — o Output Guard é pós-resposta (auditoria), não porteiro.
 *
 * trustLevel:
...
const CAP = (o) => ({
  supportsPreOutputInterception: false, // nenhuma CLI intercepta o render do harness
...
export const HARNESS_CAPABILITIES = Object.freeze({
  claude: CAP({ id: "claude", mode: "hooked", supportsPreToolUse: true, supportsStopHook: true, trustLevel: "strong" }),
  cursor: CAP({ id: "cursor", mode: "hooked", supportsPreToolUse: true, supportsStopHook: true, trustLevel: "strong" }),
  opencode: CAP({ id: "opencode", mode: "native", supportsPreToolUse: true, supportsStopHook: true, trustLevel: "strong" }),
  codex: CAP({ id: "codex", mode: "hooked", supportsPreToolUse: false, supportsStopHook: true, trustLevel: "partial" }),
